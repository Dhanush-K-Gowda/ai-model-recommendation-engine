...
class TraceIngestionSerializer(serializers.Serializer):
    ...
    application_id = serializers.CharField(
        required=True, help_text="Unique identifier for the application/service"
    )
    prompt = serializers.CharField(
        required=True, allow_blank=True, help_text="The input prompt sent to the model"
    )
    response = serializers.CharField(
        required=True, allow_blank=True, help_text="The response from the model"
    )
    ...
    latency_sec = serializers.FloatField(
        required=False, min_value=0, help_text="Latency in seconds"
    )
    ...
    traced_at = serializers.DateTimeField(
        required=False, help_text="Timestamp when the trace occurred (defaults to now)"
    )
    ...
    def create(self, validated_data):
        ...
        application, _ = Application.objects.get_or_create(
            application_id=application_id, defaults={"name": application_id}
        )
        ...
        total_tokens = validated_data.get(
...
class TraceBulkIngestionSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        """Create multiple LLMTrace instances using bulk_create."""
        traces_data = validated_data["traces"]

        # Collect unique application_ids for batch lookup
        application_ids = set(
            td.get("application_id") for td in traces_data if td.get("application_id")
        )

        # Batch get_or_create applications
        applications_cache = {}
        for app_id in application_ids:
            app, _ = Application.objects.get_or_create(
                application_id=app_id, defaults={"name": app_id}
            )
            applications_cache[app_id] = app

        # Resolve all model names in bulk
        model_names = set(
            td.get("model_name") for td in traces_data if td.get("model_name")
        )
        models_cache = {}
        for model_name in model_names:
            models_cache[model_name] = ModelResolver.resolve(model_name)

        # Prepare traces for bulk creation
        trace_objects = []
        errors = []
        ...
        for idx, trace_data in enumerate(traces_data):
            ...
            if serializer.is_valid():
                ...
                app_id = validated.get("application_id")
                application = applications_cache.get(app_id)

                # Get model from cache
                model_name = validated.get("model_name")
                resolved_model = models_cache.get(model_name)

                # Calculate tokens if needed
                input_tokens = validated.get("input_token_count")
                output_tokens = validated.get("output_token_count")

                if not input_tokens:
                    input_tokens = serializer.estimate_tokens(
                        validated.get("prompt", "")
                    )
                if not output_tokens:
                    output_tokens = serializer.estimate_tokens(
                        validated.get("response", "")
                    )

                total_tokens = validated.get(
                    "total_token_count", input_tokens + output_tokens
                )

                # Calculate costs
                input_cost = validated.get("input_cost", Decimal("0"))
                output_cost = validated.get("output_cost", Decimal("0"))
                total_cost = validated.get("total_cost", Decimal("0"))
                ...
                trace_objects.append(
                    LLMTrace(
                        ...
                        output_cost=output_cost,
                        total_cost=total_cost,
                        estimated_latency_sec=validated.get("latency_sec"),
                        tool_used=validated.get("tool_used", False),
                        status=validated.get("status", "success"),
                        category=validated.get("category", ""),
                        traced_at=validated.get("traced_at", timezone.now()),
                    )
                )
            else:
                errors.append({"index": idx, "errors": serializer.errors})

        # Bulk create all traces at once
        if trace_objects:
            LLMTrace.objects.bulk_create(trace_objects)

        # Store errors for potential retrieval
        self.errors = errors
