...
from engine.models import (
    Application, LLMTrace, AIModel, UsageAnalysis, Recommendation, Provider
)
...
def applications_list(request):
    ...
    try:
        ...
        # Get recent usage analysis for each application
        recent_analyses = {}
        for analysis in UsageAnalysis.objects.filter(
            application__in=applications
        ).select_related('application', 'model').order_by('-analysis_period_end'):
        ...
        for app in applications:
            ...
            total_cost = sum(float(t.total_cost) for t in traces)
            total_tokens = sum(t.total_token_count for t in traces)
            ...
            if analysis:
                usage = {
                    ...
                    'avg_latency_sec': float(analysis.avg_latency_sec) if analysis.avg_latency_sec else None
                }
...
def application_detail(request, application_id):
    ...
    try:
        ...
        usage_analysis = None
        latest_analysis = UsageAnalysis.objects.filter(
            application=app
        ...
        if latest_analysis:
            usage_analysis = {
                ...
                'avg_latency_sec': float(latest_analysis.avg_latency_sec) if latest_analysis.avg_latency_sec else None
            }
        ...
        for rec in Recommendation.objects.filter(
            ...
            is_active=True,
            is_dismissed=False
        ).select_related('recommended_model', 'recommended_model__provider').order_by('-confidence_score')[:10]:
            recommendations.append({
                'id': rec.id,
                'application_id': app.application_id,
                'application_name': app.name,
                'current_model': rec.current_model_name,
                'recommended_model': rec.recommended_model.name if rec.recommended_model else None,
                'provider': rec.recommended_model.provider.name if rec.recommended_model else None,
                ...
                'quality_overall_score': float(rec.quality_overall_score) if rec.quality_overall_score else None,
                'features': rec.recommended_model.categories if rec.recommended_model and rec.recommended_model.categories else []
            })
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'name': app.name or app.application_id,
                'categories': app.categories or [],
                'assigned_model': app.assigned_model if hasattr(app, 'assigned_model') else None,
                'is_active': app.is_active,
                'stats': {
                    'total_requests': total_requests,
        ...
        try:
            if application_id:
                ...
                recommendations = Recommendation.objects.filter(
                    ...
                    is_active=True,
                    is_dismissed=False
                ).select_related('application', 'recommended_model', 'recommended_model__provider').order_by('-confidence_score')
            else:
                recommendations = Recommendation.objects.filter(
                    is_active=True,
                    is_dismissed=False
                ).select_related('recommended_model', 'recommended_model__provider', 'application').order_by('-confidence_score')
        ...
        try:
            ...
            for model in models:
                ...
                avg_cost_per_1m = float(
                    (model.pricing.request_token_price + model.pricing.response_token_price) / 2
                )
                ...
                if model.benchmark_scores:
                    ...
                    total_score = 0
                    total_weight = 0
                    benchmark_weights = {'MMLU': 0.4, 'HumanEval': 0.4, 'SWE-bench Verified': 0.2}
...
def generate_recommendations_for_app(request):
    ...
    try:
        ...
        clear_existing = data.get('clear_existing', True)  # Default to clearing old recommendations
        ...
        recommendations = Recommendation.objects.filter(
            ...
            is_active=True,
            is_dismissed=False
        ).select_related('recommended_model', 'recommended_model__provider').order_by('-confidence_score')
...
def dashboard_stats(request):
    ...
    try:
        ...
        apps_with_recommendations = Application.objects.filter(
            recommendations__is_active=True,
            recommendations__is_dismissed=False
        ).distinct().count()
